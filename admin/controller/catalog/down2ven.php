<?php  
class ControllerCatalogDown2Ven extends Controller {  
	private $error = array();
   
  	public function index() {
		$this->load->language('catalog/down2ven');
  	...
  	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_down2ven->addDownload($this->request->post);
			...
			$this->redirect($this->url->link('catalog/down2ven', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
  	...
  	public function update() {
    	...
    	if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm() && $this->validateUpdate()) {
			$this->model_catalog_down2ven->editDownload($this->request->get['download_id'], $this->request->post);
  	...
  	public function delete() {
    	...
    	if (isset($this->request->post['selected']) && $this->validateDelete()) {	  
			foreach ($this->request->post['selected'] as $download_id) {
				$this->model_catalog_down2ven->deleteDownload($download_id);
			}
  	...
  	private function getList() {
		...
		if (isset($this->request->get['page'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('catalog/down2ven', 'token=' . $this->session->data['token'] . $url, 'SSL'),
      		'separator' => ' :: '
   		);
							
		$this->data['insert'] = $this->url->link('catalog/down2ven/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['delete'] = $this->url->link('catalog/down2ven/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');	
		...
		$download_total = $this->model_catalog_down2ven->getTotalDownloads();
	
		$results = $this->model_catalog_down2ven->getDownloads($data);
    	...
    	foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('catalog/down2ven/update', 'token=' . $this->session->data['token'] . '&download_id=' . $result['download_id'] . $url, 'SSL')
			);
		...
		$this->data['sort_name'] = $this->url->link('catalog/down2ven', 'token=' . $this->session->data['token'] . '&sort=dd.name' . $url, 'SSL');
		$this->data['sort_remaining'] = $this->url->link('catalog/down2ven', 'token=' . $this->session->data['token'] . '&sort=d.remaining' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('catalog/down2ven', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->template = 'catalog/down2ven_list.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  	...
  	private function getForm() {
		...
		if (!isset($this->request->get['download_id'])) {
			$this->data['action'] = $this->url->link('catalog/down2ven/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('catalog/down2ven/update', 'token=' . $this->session->data['token'] . '&download_id=' . $this->request->get['download_id'] . $url, 'SSL');
		}
		
		$this->data['cancel'] = $this->url->link('catalog/down2ven', 'token=' . $this->session->data['token'] . $url, 'SSL');
    	...
    	if (isset($this->request->get['download_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$download_info = $this->model_catalog_down2ven->getDownload($this->request->get['download_id']);
    	}
		...
		if (isset($this->request->post['download_description'])) {
			...
			$this->data['download_description'] = $this->model_catalog_down2ven->getDownloadDescriptions($this->request->get['download_id']);
		} else {
		...
		if (isset($this->request->post['vendor_id'])) {
			...
			$this->data['vendor_id'] = $download_info['vendor_id'];
		} else {
      		$this->data['vendor_id'] = 0;
    	}
		/*code end*/

		$this->template = 'catalog/down2ven_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  	...
  	private function validateForm() { 
    	if (!$this->user->hasPermission('modify', 'catalog/down2ven')) {
	...
	private function validateUpdate() {
		...
		$this->load->model('catalog/down2ven');
    	$isDLexist = $this->model_catalog_down2ven->ValidateDLUpdate($this->request->get['download_id']);
