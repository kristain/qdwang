<?php
class ControllerReportPro2VenViewed extends Controller {
	public function index() {     
		...
		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('report/pro2ven_viewed', 'token=' . $this->session->data['token'] . $url, 'SSL'),
      		'separator' => ' :: '
   		);		
	...
	public function reset() {
		...
		$this->model_report_pro2ven_report->reset();
		
		$this->session->data['success'] = $this->language->get('text_success');
		
		$this->redirect($this->url->link('report/pro2ven_viewed', 'token=' . $this->session->data['token'], 'SSL'));
	}
}
?>