...
class ModelSaleCustomer extends Model {
	...
	public function getCustomers($data = array()) {
		$sql = "SELECT *, CONCAT(c.firstname) AS name, cgd.name AS customer_group,addr.company AS shop_name,count(distinct CONCAT(c.firstname)) FROM " . DB_PREFIX . "customer c LEFT JOIN " . DB_PREFIX . "customer_group_description cgd ON (c.customer_group_id = cgd.customer_group_id) LEFT JOIN " . DB_PREFIX . "address addr ON (c.customer_id = addr.customer_id) WHERE cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		...
		if (!empty($data['filter_shop_name'])) {
			$implode[] = "LCASE(addr.company) LIKE '%" . $this->db->escape(utf8_strtolower($data['filter_shop_name'])) . "%'";
		}
	...
	public function getTotalCustomers($data = array()) {
      	$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer c LEFT JOIN " . DB_PREFIX . "address addr ON (c.customer_id = addr.customer_id)";
	...
	public function getTotalAmount2($customer_id) {
	...
	public function addTransaction2($customer_id, $description = '', $amount = '') {
