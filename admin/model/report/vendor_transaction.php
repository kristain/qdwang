...
class ModelReportVendorTransaction extends Model {
	public function getVendorOrders($data = array()) {
		$sql = "SELECT op.product_id AS product_id, o.date_added AS date, o.order_id AS order_id, o.order_status_id AS order_status, pd.name AS product_name, op.price AS price,op.quantity AS quantity, op.commission AS commission,op.vendor_total AS amount, op.order_product_id as order_product_id, op.total AS total, op.vendor_id as vendor_id, op.vendor_paid_status AS paid_status,op.tax as tax, (op.tax*op.quantity) as total_tax, op.store_tax as store_tax, op.vendor_tax as vendor_tax FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "order_product` op ON (o.order_id = op.order_id) LEFT JOIN `" . DB_PREFIX . "product_description` pd ON (op.product_id = pd.product_id)";
		
		if (!empty($data['filter_order_status_id'])) {
			$sql .= " WHERE o.order_status_id = '" . (int)$data['filter_order_status_id'] . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		} else {
			$sql .= " WHERE o.order_status_id > '0' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		}
	...
	public function getVendorTotalAmount($data = array()) {
		...
		$sql .= " group by vendor_id,company,paypal_email";
	...
	public function getCommissionData($data = array()) {
		$query = $this->db->query("SELECT vds.vendor_id, cm.commission, cm.commission_type FROM " . DB_PREFIX . "vendors vds LEFT JOIN " . DB_PREFIX . "commission cm ON (vds.commission_id = cm.commission_id)");
		return $query->rows;
	}
