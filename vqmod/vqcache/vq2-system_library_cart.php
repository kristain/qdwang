		...
		if (!$this->data) {
			foreach ($this->session->data['cart'] as $key => $quantity) {
				...
				if ($product_query->num_rows) {
					...
					//获取vendor信息
					$query2 = $this->db->query("SELECT t1.vendor_id AS vendorId FROM " . DB_PREFIX . "vendors t1 LEFT JOIN " . DB_PREFIX . "vendor t2 ON (t1.vendor_id = t2.vendor) where t2.vproduct_id= '" . (int)$product_id . "'");
