...
<div id="content"><?php echo $content_top; ?>
  ...
  <?php } ?>
    ...
    <div id="payment-method">
      ...
      <div class="onecheckout-content"></div>
	  <input type='hidden' name="instId" id="instId" value="CMBCHINA-NET-B2C">
    </div>
  </div>
		...
		success: function(json) {
			...
			if (json['output']) {
				if (json['total']) {
					...
					$('#products_total').val(json['products_total']);
				...
				$.ajax({
					...
					success: function(json) {
						...
						if (json['paymentmethod']) {
							...
							var bankcode = $("#instId").val();
							var bankname = '';
							var paymentmethod = $("#paymentMethodTitle").html();
							if(paymentmethod=='银行卡易宝支付'){
							if(bankcode=='ICBC-NET-B2C'){
							   bankname = '<img src="images_bank/gonghang.jpg" width="60%">';
							}else if(bankcode=='CCB-NET-B2C'){
							   bankname = '<img src="images_bank/jianhang.jpg" width="60%">';
							}else if(bankcode=='BOC-NET-B2C'){
							   bankname = '<img src="images_bank/zhonghang.jpg" width="60%">';
							}else if(bankcode=='ABC-NET-B2C'){
							   bankname = '<img src="images_bank/nonghang.jpg" width="60%">';
							}else if(bankcode=='CMBCHINA-NET-B2C'){
							   bankname = '<img src="images_bank/zhaohang.jpg" width="60%">';
							}else if(bankcode=='POST-NET-B2C'){
							   bankname = '<img src="images_bank/youzheng.jpg" width="60%">';
							}else if(bankcode=='BOCO-NET-B2C'){
							   bankname = '<img src="images_bank/jiaohang.jpg" width="60%">';
							}else if(bankcode=='CIB-NET-B2C'){
							   bankname = '<img src="images_bank/xingye.jpg" width="60%">';
							}
							}
							$("#paymentMethodTitle").html($("#paymentMethodTitle").html()+bankname);
						}
...
$('#button-confirmorder').live('click', function() {
  if($('input[name="payment_method"]:checked').val()=='yeepay' && $('input[name="pd_FrpId"]:checked').val()==undefined){
    alert("请选择支付银行");
	return;
  }
//var amount = $('#products_total').val();
  //if(parseFloat(amount)<25){
   // if(confirm("买够15元免服务费。本次不足15，需付1元；点“确定”提交订单；点“取消”返回继续挑选")){
	//   doConfirmorder();
	//}else{
	//  top.location.href = "http://www.qdwang.cn:80/lyhpfb/index.php?route=common/home";
	//}
 //}else{
    doConfirmorder();
 //}
