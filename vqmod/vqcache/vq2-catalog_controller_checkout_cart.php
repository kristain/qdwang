    	...
    	if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			$products = $this->cart->getProducts();
      		...
      		foreach ($products as $product) {
				...
				// 产品描述信息
				$this->data['product_description'] = $this->cart->getProductDescriptions($product['product_id']);
				...
				'cost' => $product['cost'],
					...
					'price'    => $price,
					'total'    => $total,
					'tag'      => $this->data['product_description']['1']['tag'],
		...
		if ($product_info) {			
			...
			foreach ($product_options as $product_option) {
				if ($product_option['required'] && empty($option[$product_option['product_option_id']])) {
					$json['error']['option'][$product_option['product_option_id']] = sprintf($this->language->get('error_required'), '请在'.$product_option['name'].'中选择一个商品.');
				}
			}
