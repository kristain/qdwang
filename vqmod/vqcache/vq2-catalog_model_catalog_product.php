...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		//获取vendor信息
		$query2 = $this->db->query("SELECT t1.vendor_name AS vendorName,t1.vendor_image AS vendorImage,t1.address_1 AS address,t1.position AS position FROM " . DB_PREFIX . "vendors t1 LEFT JOIN " . DB_PREFIX . "vendor t2 ON (t1.vendor_id = t2.vendor) where t2.vproduct_id= '" . (int)$product_id . "'");
		...
		$address = '西湖区庆隆横路15号';
		//默认的商家位置是自营的，经纬度是庆隆宛
		$position = '120.128033,30.303159';
		//距离，单位米
		$distance = "10000000";
		
		if($query2->num_rows){
			$vendorName = $query2->row['vendorName'];
			$vendorImage = $query2->row['vendorImage'];
		...
		if(isset($_COOKIE['position'])){
			$cusPosition = $_COOKIE['position'];
			$shopJWD = explode(',',$position);
			$custJWD = explode(',',$cusPosition);
			if(count($shopJWD)==2 && count($custJWD)==2){
			  $distance = $this->customer->getDistance($shopJWD[0],$shopJWD[1],$custJWD[0],$custJWD[1]);
			  if($distance>=1000){
	...
	public function getIsbnByProductId($product_id) {
