	...
	private $error = array();
  	...
  	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if((int)$order_info['order_status_id']==5){
			  $this->load->model('sale/customer');
			  //1.先计算该客户当前的可用余额信息
			  $balance = (float)$this->model_sale_customer->getTotalAmount2($order_info['customer_id']);
			  ...
			  if($balance>0){
				 ...
				 $this->model_sale_customer->addTransaction2($order_info['customer_id'],'扣除','-'.$minus);
			  }
			  //3.加上本次订单金额的1%的返点
			  $add = $order_total * 0.008;
			  $this->model_sale_customer->addTransaction2($order_info['customer_id'],'返点',$add);
			  
			  //4.清空购物车(缓存中)
			  $this->session->data['cart'] = array();
		      $this->data = array();
			  //4.清空购物车(数据库中)
			  $this->model_sale_customer->cleanCartFromDB($order_info['customer_id']);
			}
		...
		$subject = $this->language->get('text_update_subject') ;
		$message = $order_info['firstname']. "  ".$this->language->get('text_update_order') ."#".$order_info['order_id']."  ";
		$message .= $this->language->get('text_new_total') . $order_info['total'] . "元 ". "\n";
		$message .= $this->language->get('text_update_date_added')."#". $order_info['date_added']. "\n";
		$message .= $this->language->get('text_update_order_status'). "\n" ;
		if ($order_info['customer_id']) {
			$message .= $this->language->get('text_new_link_transaction') . "\n";
			$message .= $data['store_url'] . 'index.php?route=account/transaction'. "\n\n";
		}
  	...
  	private function getList() {
    	...
    	foreach ($results as $result) {
			...
			$this->data['orders'][] = array(
				...
				'date_added'    => date($this->language->get('datetime_format_long'), strtotime($result['date_added'])),
				'date_modified' => date($this->language->get('datetime_format_long'), strtotime($result['date_modified'])),
  	...
  	public function getForm() {
		...
		foreach ($order_products as $order_product) {
			...
			$isbn = $this->model_sale_order->getIsbnByProductId($order_product['product_id']);
			$sku = $this->model_sale_order->getSkuByProductId($order_product['product_id']);
	...
	public function info() {
		...
		if ($order_info) {
			...
			foreach ($products as $product) {
                ...
                $isbn = $this->model_sale_order->getIsbnByProductId($product['product_id']);
				$sku = $this->model_sale_order->getSkuByProductId($product['product_id']);
		...
		$json = array();
     	...
     	if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			if ($order_info && $order_info['affiliate_id']) {
				...
				if (!$affiliate_total) {
					$this->model_sale_affiliate->addTransaction($order_info['affiliate_id'], $this->language->get('text_order_id') . ' #' . $this->request->get['order_id'].' '.$order_info['payment_company'].' 订单金额:'.$order_info['total'], $order_info['commission'], $this->request->get['order_id']);
