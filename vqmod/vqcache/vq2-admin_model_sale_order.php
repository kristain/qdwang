		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "order_download WHERE order_id = '" . (int)$order_id . "'");
      	...
      	if (isset($data['order_product'])) {		
      		foreach ($data['order_product'] as $order_product) {
                ...
                //获取vendor信息
				$query2 = $this->db->query("SELECT t1.vendor_id AS vendorId FROM " . DB_PREFIX . "vendors t1 LEFT JOIN " . DB_PREFIX . "vendor t2 ON (t1.vendor_id = t2.vendor) where t2.vproduct_id= '" . (int)$order_product['product_id'] . "'");
				
				$vendorId = '0';
				
				if($query2->num_rows){
					$vendorId = $query2->row['vendorId'];
				}			
			...
			//更新订单中产品的cost信息
		    $this->db->query("UPDATE `" . DB_PREFIX . "order_product` op INNER JOIN `" . DB_PREFIX . "product` p ON p.product_id = op.product_id SET op.cost = p.cost where op.order_id='" . (int)$order_id . "'");
		}
