...
<div id="content">
  ...
  <div class="box">
    ...
    <div class="content">
      <!--code start - modify -->
	  <!--add -->
      <div id="tabs" class="htabs"><a href="#tab-general"><?php echo $tab_general; ?></a><a href="#tab-data"><?php echo $tab_data; ?></a><?php if ($this->config->get('vendor_tab')) { ?><a href="#tab-vendor"><?php echo $tab_vendor; ?></a><?php } ?><a href="#tab-links"><?php echo $tab_links; ?></a><a href="#tab-attribute"><?php echo $tab_attribute; ?></a><a href="#tab-option"><?php echo $tab_option; ?></a><a href="#tab-discount"><?php echo $tab_discount; ?></a><a href="#tab-special"><?php echo $tab_special; ?></a><a href="#tab-image"><?php echo $tab_image; ?></a><?php if ($this->config->get('vendor_reward_points')) { ?><a href="#tab-reward"><?php echo $tab_reward; ?></a><?php } ?><?php if ($this->config->get('vendor_desgin_tab')) { ?><a href="#tab-design"><?php echo $tab_design; ?></a><?php } ?></div>
      <!--code end-->
	  <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
          ...
          <div id="language<?php echo $language['language_id']; ?>">
            <table class="form">
              <tr>
                ...
                <td nowarp><input type="text" id="productName" name="product_description[<?php echo $language['language_id']; ?>][name]" size="100" value="<?php echo isset($product_description[$language['language_id']]) ? $product_description[$language['language_id']]['name'] : ''; ?>" />
                  <?php if (isset($error_name[$language['language_id']])) { ?>
                  <span class="error"><?php echo $error_name[$language['language_id']]; ?></span>
                  <?php } ?>
				   <input type="hidden" id="copyProduceId" name="copyProduceId">
				   <?php if (isset($isInsert)) { ?>
	 	              <a onclick="copyProduct()" class="button">产品复制</a>
		           <?php } ?>
				  </td>
              </tr>
              <tr>
		...
		<!--code start-->
		<!--add -->
		<?php if ($this->config->get('vendor_tab')) { ?>
		<div id="tab-vendor">
			<table class="form">
			<tr>
				<td><?php echo $entry_vendor_name; ?></td>
				<td><?php foreach ($vendors as $vend) { ?>
					<?php if ($vendor) { ?>
						<?php if ($vend['vendor_id'] == $vendor) { ?>
							<?php echo $vend['vendor_name']; ?>
		  ...
		  <tr>
            ...
            <td><input type="text" name="vtotal" value="<?php echo $vtotal; ?>" onKeyUp="procost(this.form)" /></td>
		  </tr>
		  <tr>
		...
		<?php if ($this->config->get('vendor_reward_points')) { ?>
        <div id="tab-reward">
          <table class="form">
            <tr>
		...
		<?php } ?>
		<?php if ($this->config->get('vendor_desgin_tab')) { ?>
        <div id="tab-design">
          <table class="list">
            <thead>
              <tr>
...
function getVendors() {
	$.ajax({
		url: 'index.php?route=catalog/pro2ven/vendor&token=<?php echo $token; ?>&vendor_id=' + $('#vendor').attr('value'),
		dataType: 'json',
...
$('#productName').autocomplete({
	...
	select: function(event, ui) {
		$('#productName').val(ui.item.label+ui.item.value);copyProduceId
		$('#copyProduceId').val(ui.item.value);				
		return false;
	},
	focus: function(event, ui) {
      	return false;
...
function copyProduct(){
  var newlocation = "<?php echo html_entity_decode($action); ?>";
  var copyProductId = $('#copyProduceId').val();
  if(copyProductId==''){
     alert("请先选择商品.");
	 return;
  }
  location = newlocation +"&copy_product_id="+copyProductId;
}
//--></script> 
<!--code end--> 
<?php echo $footer; ?>